...
window.onload = function() {
	...
	//voor alle 54 Europese landen
	for (i = 0; i < JSON_data.countries.length; i++)
	{	
		//ga door de lijst van alle landen (met countrycodes)
		for (j = 0; j < country_codes.length; j++)
		{	

			//als europees land (HTML) overeenkomt met variable index countrycode 
			if(JSON_data.countries[i].name == country_codes[j][2])
			{	

				console.log(JSON_data.countries[i].name);
				//neem de kleur
				color_now = Choose_Color(JSON_data.countries[i].points);
				console.log(color_now);
				//kleur het land in op de kaart met invoerwaarden 'id' IN LOWERCASE, desbetreffende kleur
				changeColor(country_codes[j][0], color_now);
			}	
		}
	}	
} 
